...
int main(int ac, char **av)
{
    ...
    while (1)
    {
        ...
        for (int fd = 0; fd <= max_fd; fd++)
        {
            if (FD_ISSET(fd, &read_fds) && fd == sockfd) // new connection (socketfd) (test if the fd is a part of the read_fds and if it's the socketfd (so it is a new one)) AND it is new one // so handle the new connection
            {
                int cl_sock = accept(sockfd, NULL, NULL); // accept the new connection
                if (cl_sock < 0) // valid fd
                    continue;
                if (cl_sock > max_fd) // update the  max_fd
                    max_fd = cl_sock;
                clients[cl_sock].id = next_id++; // add new one to the struct of the clients ids
                bzero(clients[cl_sock].msg, sizeof(clients[cl_sock].msg));
                FD_SET(cl_sock, &active_fds); // set the new one to the active fds
                sprintf(buffWrite, "server: client %d just arrived\n", clients[cl_sock].id);
                sendMsg(cl_sock); // broadcast the the message to all clents except the new one (cl_sock)
                break;
            }
            else if (FD_ISSET(fd, &read_fds)) // check if the fd is part of the set (read_fds) 
            {
